# File: isobmff/atoms/stsz.py

from . import FullAtom, Atom, Table


class StszAtom(FullAtom):
    """
    Class representing the 'stsz' atom in an ISO Base Media File.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.properties.update({"sample_size": None, "entry_count": None, "entries": None})
        self.sample_size = self._type_registry["int"](
            None, self._read_slice(slice(0, 4))
        )
        self.entry_count = self._type_registry["int"](
            None, self._read_slice(slice(4, 8))
        )
        self._header_size += 8
        self.entries = Table(
            None,
